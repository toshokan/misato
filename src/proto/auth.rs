...
impl<'i> Parser<'i> {
    ...
    fn parse_client_data(input: &'i str) -> IResult<&'i str, ClientCommand<'i>> {
        ...
        let (input, data) = preceded(tag("DATA"), preceded(char(' '), take_until("\r\n")))(input)?;
    ...
    fn parse_client_error(input: &'i str) -> IResult<&'i str, ClientCommand<'i>> {
        ...
        let (input, description) =
            preceded(tag("ERROR"), opt(preceded(char(' '), take_until("\r\n"))))(input)?;
            ...
            match self.state {
                AuthState::WaitingForAuth => {
                    ...
                    match cmd {
                        ...
                        ClientCommand::Auth { .. } => {
                            self.write_server_command(ServerCommand::Ok(
                                "<SERVER GUID HERE>".to_string(),
                            ))?; // TODO
                ...
                AuthState::WaitingForData => {
                    ...
                    match cmd {
                        ...
                        ClientCommand::Cancel | ClientCommand::Error(_) => {
                            self.write_server_command(ServerCommand::Rejected(
                                Mechanism::External,
                            ))?;
